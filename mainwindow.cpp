    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(ui->showhistory, &QAction::triggered, this, &MainWindow::onshowhistory);
    connect(ui->hidehistory, &QAction::triggered, this, &MainWindow::onhidehistory);
...
void MainWindow::onclearhistory() {
    historylist.clear();
    historyview->clear();
    statusBar()->showMessage("История очищена", 3000);
}
...
void MainWindow::onsaveaction() {
    ...
    if(currentValue.isEmpty()) {
        statusBar()->showMessage("Значение пустое не добавлено в историю", 3000);
    }
    historylist.append(currentValue);
    historyview->clear();
    historyview->addItems(historylist);

    statusBar()->showMessage("Значение" + currentValue + "Добавленно", 3000);

}
void MainWindow::onshowhistory() {
    if(historywidget->isVisible()) return;
    this->setFixedSize(606, 309);
    historywidget->show();
}
